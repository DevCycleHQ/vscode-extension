...
export default class VariablesCLIController extends DevcycleCLIController {
  public static async getVariable(variableKey: string) {
    ...
    const { code, error, output } = await DevcycleCLIController.execDvc(`variables get --keys='${variableKey}'`)
    if (code !== 0) {
  ...
  public static async getAllVariables() {
    const variables = StateManager.getState(KEYS.VARIABLES) as Record<string, Variable>
    if (variables) {
      return variables
    }
    const { code, error, output } = await DevcycleCLIController.execDvc('variables get')
    if (code !== 0) {
