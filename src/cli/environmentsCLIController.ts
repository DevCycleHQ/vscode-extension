...
import DevcycleCLIController from './baseCLIController'
...
export default class EnvironmentsCLIController extends DevcycleCLIController {
  public static async getEnvironment(environmentId: string) {
    ...
    const { code, error, output } = await DevcycleCLIController.execDvc(`environments get --keys='${environmentId}'`)
    if (code !== 0) {
  ...
  public static async getAllEnvironments() {
    const environments = StateManager.getState(KEYS.ENVIRONMENTS) as Record<string, Environment>
    if (environments) {
      return environments
    }
    const { code, error, output } = await DevcycleCLIController.execDvc('environments get')
    if (code !== 0) {
