...
export default class FeaturesCLIController extends DevcycleCLIController {
  public static async getFeature(featureId: string) {
    const features = StateManager.getState(KEYS.FEATURES) as Record<string, Feature> || {}
    const feature = features[featureId]
    if (feature) {
      return feature
    }
    const { code, error, output } = await DevcycleCLIController.execDvc(`features get --keys='${featureId}'`)
    if (code !== 0) {
  ...
  public static async getAllFeatures() {
    ...
    const { code, error, output } = await DevcycleCLIController.execDvc('features get')
    if (code !== 0) {
  ...
  public static async getFeatureConfigurations(featureId: string) {
